#!/usr/bin/env python3
"""
Tuned debug Ishihara MCQ generator (mask font x3, bg dots half-size)

Usage:
  python3.13 mcq_debug_tuned.py --count 3 --out debug_out
...
# ---------- Tunable params (user-requested changes applied) ----------
WIDTH, HEIGHT = 700, 300
DOT_PITCH = 12
JITTER = 2

# Halved background blob size (was 10 -> now 5)
BG_DOT_SIZE = 5

# foreground dot size (keep or tweak as needed)
FG_DOT_SIZE = 14

# Mask font multiplier (triple the mask text size)
MASK_FONT_MULTIPLIER = 3

MASK_THRESHOLD = 0.25  # keep same; can be tuned later
...
def make_mask(figure_text, w, h, padding=12):
    ...
    # choose font (base size scaled then multiplied)
    base_size = int(h * 0.22)            # original base
    size = max(12, int(base_size * MASK_FONT_MULTIPLIER))  # apply multiplier
    if FONT_PATH:
        try:
    ...
    # shrink font if absolutely necessary to fit (rare with multiplier)
    while True:
        wmax = 0; hsum = 0
        for ln in lines:
        ...
        if (wmax < w - 2*padding) and (hsum < h - 2*padding):
            break
        # if too large, step down in size
        if getattr(font, "size", None) and FONT_PATH and font.size > 12:
            font = ImageFont.truetype(FONT_PATH, max(12, font.size - 4))
        else:
            break
