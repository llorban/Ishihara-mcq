...
Save as mcq_gen.py and run: python mcq_gen.py --count 6 --zip

Dependencies:
    pip install pillow numpy
"""
from PIL import Image, ImageDraw, ImageFont, ImageFilter
import numpy as np, os, random, textwrap, zipfile, argparse, sys
...
def text_size(draw, text, font):
    ...
    - prefer draw.textbbox (most reliable)
    - fall back to font.getbbox or font.getsize if available
    ...
    try:
        ...
        # fallback estimate
        return (int(len(text) * (getattr(font, "size", 10) * 0.6)), getattr(font, "size", 12))
