...
<img src="@@PLUGINFILE@@/{html.escape(image_filename)}" alt="{html.escape(qname)}" />"""
    ...
    # ddmarker expects top-left x,y and width,height (pixels)
    xml_parts.append(f'      <x>{left}</x>')
    xml_parts.append(f'      <y>{top}</y>')
    xml_parts.append(f'      <width>{size}</width>')
    xml_parts.append(f'      <height>{size}</height>')
    # mark this as the correct target (plugin may expect an id/correct flag)
    xml_parts.append(f'      <id>1</id>')
    xml_parts.append(f'      <correct>1</correct>')
    xml_parts.append('    </targetbox>')
    # include file data
