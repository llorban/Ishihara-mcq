...
def find_image_for_json_strict(jf_path: Path, img_dir: Path):
    """
    Strict matching: for JSON file 'X.json' find images named exactly 'X.<ext>'
    in img_dir (where ext in ALLOWED_EXTS). Returns (chosen_path or None, matches_list).
    - If zero matches -> (None, []).
    - If one match -> (Path, [Path]).
    - If multiple -> returns chosen Path by PREFERRED_ORDER and returns full match list.
      Also caller should log a warning so user can resolve duplicates.
    """
    stem = jf_path.stem  # 'X' for X.json
    matches = []
    ...
    if not matches:
        return None, []
    # deterministic choose: prefer extension order in PREFERRED_ORDER
    for pref in PREFERRED_ORDER:
        for m in matches:
            if m.suffix.lower().lstrip('.') == pref.lower().lstrip('.'):
                return m, matches
    # fallback (shouldn't happen) - return first sorted
...
<img src="@@PLUGINFILE@@/{html.escape(image_filename)}" alt="{html.escape(qname)}" />"""
    ...
    # marker label as metadata
    xml_parts.append('    <dragmarker>')
    xml_parts.append(f'      <markerlabel>{html.escape(marker_label)}</markerlabel>')
    xml_parts.append('    </dragmarker>')
    # target square metadata
    xml_parts.append('    <targetbox>')
    xml_parts.append(f'      <left>{left}</left>')
    xml_parts.append(f'      <top>{top}</top>')
    xml_parts.append(f'      <size>{size}</size>')
    xml_parts.append(f'      <right>{left + size}</right>')
    xml_parts.append(f'      <bottom>{top + size}</bottom>')
    xml_parts.append('    </targetbox>')
    # include file data
...
def main():
    ...
    for jf in json_files:
        ...
        # --- strict-match usage (replace prior candidate logic) ---
        ...
        if image_path is None:
            ...
            continue
        if len(matches) > 1:
            print(f"WARNING: Multiple image files matched for {jf.name}: {[m.name for m in matches]}. Using {image_path.name}.")
        # --- end strict-match usage ---
        ...
        csv_rows.append([image_name, jf.name, qname, correct_idx, left_sq, top_sq, size, right_sq, bottom_sq])
    ...
    with open(csv_out, "w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow(["image","json_file","qname","correct_index","left","top","size","right","bottom"])
