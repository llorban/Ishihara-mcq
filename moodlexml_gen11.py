...
def main():
    ...
    for jf in json_files:
        ...
        csv_rows.append({
            ...
            'rect_left': int(left), 'rect_top': int(top), 'rect_width': int(width), 'rect_height': int(height), 'rect_bottom': int(top + height)
        })

        if not args.quiet:
            print(f"Added {jf.name} -> image {image_name} ... box L,T,W,H = {left},{top},{width},{height}")
